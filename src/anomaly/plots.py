...
def plot_anomaly_detection(df: pd.DataFrame, path: str, title: str = "") -> None:
    ...
    ax1.plot(dates, df["y_true_original"], label="OT (clean)",
             color="#333333", alpha=0.6)
    ax1.plot(dates, df["y_true_anomalous"], label="OT (with anomalies)",
             color=PALETTE[0], alpha=0.85)
    ax1.plot(dates, df["y_pred"], label="prediction",
             color=PALETTE[2], alpha=0.7)
    ax1.scatter(dates[is_anom], df["y_true_anomalous"].to_numpy()[is_anom],
                color=PALETTE[1], s=16, zorder=5, label="true anomaly")
    ax1.set_ylabel("Oil Temperature (OT)")
    ax1.set_title(title)
    ax1.legend(loc="upper right")

    ax2.plot(dates, df["anomaly_score"], color=PALETTE[0], label="anomaly score")
    ax2.axhline(float(df["threshold"].iloc[0]), color=PALETTE[2], linestyle="--",
                label="threshold")
    ax2.scatter(dates[pred], df["anomaly_score"].to_numpy()[pred],
                color=PALETTE[4], s=16, zorder=5, label="detected")
